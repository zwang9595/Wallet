...
public class Card2 extends AppCompatActivity {
    ...
    private void Add() {
        ...
        if(LoginTest.loggedin) {
            User_main.db.execSQL("Update Cards set card2 = 1 where name = '" + LoginTest.username + "';");
        }
        onBackPressed();
    }
    ...
    private void Remove() {
        Wallet.card2 = 0;
        if(LoginTest.loggedin) {
            User_main.db.execSQL("Update Cards set card2 = 0 where name = '" + LoginTest.username + "';");
        }
        onBackPressed();
    }
